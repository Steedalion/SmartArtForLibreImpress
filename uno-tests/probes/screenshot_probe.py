...
def draw_sequential_chevron(doc, page):
    """3 steps: pentagon → chevron → chevron; sub-boxes placed horizontally below
    each chevron, scaled to fit within the chevron width; first sub-item of Alpha
    (Bravo) has a level-3 child (Hotel)."""
    ...
    for x, y, label, kind, subs, color in steps:
        ...
        cx = x + W // 2
        n_subs = len(subs)
        # Scale sub-item width so the row fits inside the chevron.
        sub_w = min(W - 30, (W - (n_subs - 1) * SUB_GAP) // n_subs)
        child_w = max(400, sub_w - 400)
        total_sub_w = n_subs * sub_w + (n_subs - 1) * SUB_GAP
